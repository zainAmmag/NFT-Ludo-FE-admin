...
class Invoice extends React.Component {
  ...
  searchInvoiceByFilter = async () => {
    try {
      ...
      let t = localStorage.getItem(AuthenticationTokenId);
      var data = await SendHttpRequest(
        BaseUrl + "GetRecentInvoiceByFilter",
        {
          Token: t,
      ...
      if (data.Success) {
        if (data.Data == "") {
          this.setState({
            dataSource: data.Data.InvoiceModel,
            from: "",
            to: "",
            tableData: [],
            periodModal: false,
          ...
          let d = this.populateTableData(data.Data.InvoiceModel);
          this.setState({
            dataSource: data.Data.InvoiceModel,
            from: "",
            to: "",
    ...
    return (
      <div>
         <div className="col-sm-9">
                    <h2 className="">RECENT INVOICE</h2>
          ...
          <div className="col-lg-12 col-md-12 col-xl-4 col-12 order-1 order-lg-1 order-xl-2">
            <div className="card">
              ...
              <div className="row" style={{ marginBottom: "1rem" }}>
                ...
                >
                  <p style={{ color: "#fff", marginBottom: 0 }}>
                    {this.state.AccountDetail.TotalBalance > 0
                      ? (parseFloat(
                          this.state.AccountDetail.TotalBalance
                        )/(this.state.Rate===0?1:this.state.Rate)).toFixed(2)
                      : 0.00}{" "}
                    {this.state.CurrencyName}
                    {/* {parseFloat(this.state.AccountDetail.TotalBalance).toFixed(3)} */}
                  </p>
                </div>
              </div>
              <hr></hr>
                ...
                >
                  <p style={{ color: "#fff", marginBottom: 0 }}>
                    {this.state.AccountDetail.TotalBalance > 0
                      ...
                      : 0.00}{" "}
                    {this.state.CurrencyName}
                    {/* USD {parseFloat(this.state.AccountDetail.TotalBalance).toFixed(3)} */}
                  </p>
                </div>
              </div>
              <hr></hr>
